...
@Component({
  ...
  styleUrls: ['./templates-forms.component.scss'],
})
...
export class TemplatesFormsComponent  implements OnInit {
  ...
  submitForm() {
    const formData = {
      ...
      cornVariety: (document.getElementById('cornVariety')as HTMLInputElement).value,
  ...
  const requiredFields = ['name', 'address', 'type', 'mobile', 'lenderName', 'lenderAddress', 'cicNo', 'farmLocation', 'north', 'south', 'east', 'west', 'areaPlanted', 'ppirArea', 'datePlanning', 'ppirDopds', 'ppirDoptp', 'tp', 'seedVariety', 'pcicList', 'cornVariety', 'conformedBy', 'preparedBy'];
